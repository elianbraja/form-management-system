...
class FormEntriesController < ApplicationController
  ...
  before_action :set_form_entry, only: %i[edit update]
  ...
  def form_entry_params
    params.expect(
      form_entries_update_service: [field_values: %i[form_field_id value]]
    )
  rescue ActionController::ParameterMissing
    params.expect(
      form_entries_create_service: [field_values: %i[form_field_id value]]
    )
  end
end
