...
module FormFieldValidation
  ...
  def validate_field(field_params, index)
    ...
    field_errors << 'Invalid field type' unless FormField::FIELD_TYPES.include?(field_params[:field_type])

    # Type-specific validations
    case field_params[:field_type]
    when 'string'
      if field_params[:min_length].present? && field_params[:max_length].present? && (field_params[:min_length].to_i > field_params[:max_length].to_i)
        field_errors << 'Minimum length cannot be greater than maximum length'
      end
    when 'integer'
      if field_params[:min_value].present? && field_params[:max_value].present? && (field_params[:min_value].to_i > field_params[:max_value].to_i)
