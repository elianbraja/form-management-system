...
module FieldValidation
  ...
  def validate_field_value(field_value_params, index)
    ...
    if field_value_params[:form_field_id].blank?
      ...
      if form_field.nil?
        ...
        if form_field.required? && field_value_params[:value].blank?
          field_errors << "#{form_field.name} is required"
        end
  ...
  def validate_string_field(value, form_field, field_errors)
    ...
    if validations['max_length'].present? && value.length > validations['max_length'].to_i
  ...
  def validate_integer_field(value, form_field, field_errors)
    ...
    if validations['max_value'].present? && int_value > validations['max_value'].to_i
